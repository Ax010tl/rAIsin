...
def analyze_file_extrinsic(blob, file_path, raisin_vectors):
...
def analyze_file_intrinsic(blob):
    # Literal n-grams
...
def analyze_file(file_path, raisin_vectors):
    with open(file_path, "r") as f:
        document = f.read()
        blob = TextBlob(document)
        intrinsic = analyze_file_intrinsic(blob)
        extrinsic = analyze_file_extrinsic(blob, file_path, raisin_vectors)
        return {**intrinsic, **extrinsic, "file_name": os.path.basename(file_path)}

def bulk_analysis(directory, raisin_vectors_path):
    # Store results
    results = []
    # Get vectors file
    with open(raisin_vectors_path, "r") as v:
        # Read vectors
        ...
        for file_path in os.listdir(directory):
            ...
            results.append( 
                analyze_file(os.path.join(directory, file_path), raisin_vectors) 
            )
        # Store results
        df = pd.DataFrame(results)
        df.to_csv(
            os.path.join(directory, f"results_{datetime.now().isoformat()}.csv")
        )
